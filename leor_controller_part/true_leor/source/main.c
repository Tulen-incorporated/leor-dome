...
void main(void) {
  ...
  // Тут будет происходить вся жизнь контроллера вне прерываний. Сделали работу - выключаемся до прерывания.
  // Прерывание, если потребуется, будит нас и делает код, который замкнут в этом цикле.
loop:
  __bis_SR_register(LPM0_bits | GIE); // Выключаем ЦПУ, глобально прерывания разрешены.
